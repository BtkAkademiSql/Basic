...
declare @numeric numeric(18,8)						-- decimal ile birebir ayný(decimal) 
...
--Metinsel 
declare @char char(8000)							--kullanýlmayan karakterler için " "(boþluk) karakterini ekler(string)
declare @nchar nchar(4000)							--char ile aynýdýr ve unicode destekli her karakter için 1 byte yerine 2 byte tutar(string)
...
declare @nvarchar varchar(max)						--2gb kadar veri tutabilir, unicode destekli her karakter için 1 byte yerine 2 byte tutar(string)
													--text,ntext kaldýrýlmýþ veri tipi geriye uyumluluk saðlamak için hala kullanýlýr(string)
--Tarih-saat
...
declare @datetimeoffset datetimeoffset				--0001-01-01 00:00:00.0000000 ile 9999-12-31 23:59:59.9999999 Time zone offset aralýðý -14.00/+14.00(DateTimeOffset)
...
declare @xml xml='<alan1></alan1>'					--xml içinde where ile arama yapýlabilir
declare @table table(alan1 varchar(10))				
declare @uniqueidentifier uniqueidentifier=NEWID()	--guid veri tipi için kullanýlýr(GUID)
declare @sql_variant sql_variant					--(object)
													--image kaldýrýlmýþ veri tipi geriye uyumluluk saðlamak için hala kullanýlýr